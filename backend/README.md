...
To run PostgreSQL, MinIO, migrations, and the production backend image together:
...
The Compose migration service runs `prisma migrate deploy` before the backend starts. Set `S3_PUBLIC_ENDPOINT=http://localhost:9000` for local browser uploads, or set it to the public MinIO Tunnel hostname when hosting remotely.

## Storage provider

The upload API uses pre-signed S3 URLs. Set `STORAGE_PROVIDER=minio` for local MinIO or `STORAGE_PROVIDER=s3` for AWS. Both use the same API surface; switching provider only requires changing environment variables. Objects are private and API responses include a short-lived `downloadUrl`. `S3_ENDPOINT` is the backend's internal storage endpoint; optional `S3_PUBLIC_ENDPOINT` is the browser-visible endpoint embedded in signed URLs.
...
| Users | `GET /api/members`, `GET /api/quota`, `POST /api/images/upload-url`, `POST /api/images` (or `/complete`), `GET /api/images`, `POST/DELETE /api/images/:imageId/share`, `GET /api/notifications`, `DELETE /api/notifications/:notificationId` |
...
All protected endpoints require `Authorization: Bearer <accessToken>`. There is no public sign-up endpoint. On startup, the API ensures the configured Product Owner exists. A Product Owner creates an organisation and its initial Admin; Admins create normal User accounts in their own organisation. Each new Admin and User receives a unique cryptographically generated temporary password returned only by that creation request.
...
| `DEFAULT_ACCOUNT_PASSWORD` | Password assigned only to the initial Product Owner |

The API refuses to start without a Product Owner email and a valid Product Owner password. Use a unique password and distribute it only to the intended Product Owner. Deliver generated Admin/User passwords securely; they are not stored in plaintext and cannot be retrieved again.
