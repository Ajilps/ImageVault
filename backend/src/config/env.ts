...
function requiredInteger(name: string, minimum = 1): number {
...
function defaultAccountPassword(): string {
  ...
  throw new AppError(
    `DEFAULT_ACCOUNT_PASSWORD must be configured with ${minimum} to ${maximum} characters.`,
    500,
    "CONFIGURATION_ERROR",
  );
}

function defaultProductOwnerEmail(): string {
