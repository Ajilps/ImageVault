...
export default function UsersPage() {
  ...
  const [createdAccount, setCreatedAccount] = useState<{ label: string; password: string } | null>(null);
  ...
  async function submitUser(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingUser) {
        ...
        setCreatedAccount({ label: response.user.email, password: response.temporaryPassword });
      }
      closeForm();
    } catch (caughtError) {
  ...
  async function removeUser(user: ManagedUser) {
    if (!window.confirm(`Remove ${user.name} and their uploaded images?`)) return;
    try {
  ...
  async function submitSlotAllocation(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await api.allocateUserSlots(allocationUser.id, Number(additionalSlots));
      setUsers((current) => current.map((user) => user.id === response.user.id ? response.user : user));
  ...
  return (
    <RoleGate>
      ...
      {createdAccount ? <TemporaryPasswordNotice accountLabel={createdAccount.label} password={createdAccount.password} onDismiss={() => setCreatedAccount(null)} /> : null}
      {isFormOpen ? <form onSubmit={submitUser} className="mb-7 rounded-2xl border border-indigo-100 bg-white p-5 shadow-sm"><div><h2 className="text-base font-bold text-slate-900">{editingUser ? `Edit ${editingUser.name}` : "Add a team member"}</h2><p className="mt-1 text-sm text-slate-500">{editingUser ? "Leave the password blank to keep the current password." : `New accounts receive the configured ${config?.defaultImageQuota ?? "initial"}-image quota and a unique generated password shown once.`}</p></div><div className={`mt-5 grid gap-4 ${editingUser ? "md:grid-cols-3" : "md:grid-cols-2"}`}><Input label="Full name" value={name} onChange={setName} /><Input label="Email" type="email" value={email} onChange={setEmail} />{editingUser ? <PasswordField label="New password" autoComplete="new-password" minLength={config?.passwordMinLength} maxLength={config?.passwordMaxLength} required={false} value={password} onChange={setPassword} /> : null}</div>{error ? <div className="mt-4"><Message>{error}</Message></div> : null}<div className="mt-5 flex justify-end gap-3"><button type="button" onClick={closeForm} className="rounded-xl px-4 py-2.5 text-sm font-bold text-slate-600 hover:bg-slate-100">Cancel</button><button disabled={isSubmitting} className="rounded-xl bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white disabled:opacity-60">{isSubmitting ? "Saving…" : editingUser ? "Save changes" : "Add User"}</button></div></form> : null}
